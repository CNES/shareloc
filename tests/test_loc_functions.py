...
@pytest.mark.parametrize("col_min_valid",
                         [([4.15161251e-02, 1.95057636e-01,
                            1.10977819e+00, -8.35016563e-04, -3.50772271e-02, -9.46432481e-03])])
@pytest.mark.parametrize("row_min_valid", [([0.05440845,  1.26513831, -0.36737151,
                                              -0.00229532, -0.07459378, -0.02558954])])
@pytest.mark.parametrize("col_max_valid", [([1.76451389e-02, 2.05533045e-01, 1.11758291e+00,
                                             -9.50086076e-04, -3.59923603e-02, -1.03291594e-02])])
@pytest.mark.parametrize("row_max_valid", [([0.07565692, 1.27499912, -0.36677813,
                                             -0.00252395, -0.07539624, -0.0270914])])
...
@pytest.mark.unit_tests
def test_pred_loc_inv(col_min_valid, row_min_valid, col_max_valid, row_max_valid,
                      valid_offset_lon, valid_offset_lat, valid_offset_row, valid_offset_col):
    """
    Test inverse localization
    """
    #init des predicteurs
...
#delta vt 0.5 pixel shift between physical model and rpc OTB
@pytest.mark.parametrize("id_scene, rpc, col,row, h, delta_vt",
                         [("P1BP--2018122638935449CP", "PHRDIMAP_P1BP--2018122638935449CP.XML" ,150.5,20.5,10.0,0.5),
                        ("P1BP--2017092838284574CP",
                         "RPC_PHR1B_P_201709281038045_SEN_PRG_FC_178608-001.XML" ,150.5,20.5,10.0,0.5)])
@pytest.mark.unit_tests
