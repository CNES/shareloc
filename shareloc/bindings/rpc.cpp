...
tuple<vector<double>,vector<double>,vector<double>> RPC::direct_loc_inverse_iterative(
    ...
    //-- Check columns and rows sizes -> col_norm and row_norm
    ...
    vector<double>long_out(col_norm.size());
    vector<double>lat_out(col_norm.size());
    vector<bool> filter_nan;

    tie(filter_nan, long_out, lat_out) = filter_coordinates(row_norm, col_norm, fill_nan);

    // if all coord contains Nan then return
    bool allFalse = none_of(filter_nan.begin(),
                                filter_nan.end(),
                                [](bool value) { return value; });
    if (allFalse){
        cout<<"C++ : direct_loc_inverse_iterative: All coords are Nan"<<endl;
        vector<double> alt_empty;
        return make_tuple(long_out, lat_out, alt_empty);
    }

    // Filtering Nan ~=> row_local = row[filter_nan]
    vector<double> row_local;
    vector<double> col_local;
    vector<double> alt_local;
    copy_if(row_norm.begin(),row_norm.end(),back_inserter(row_local),
    [&filter_nan, i = 0](int) mutable { return filter_nan[i++]; });//return filter_nan[i] then ++i
    copy_if(col_norm.begin(),col_norm.end(),back_inserter(col_local),
    [&filter_nan, i = 0](int) mutable { return filter_nan[i++]; });
    copy_if(alt_norm.begin(),alt_norm.end(),back_inserter(alt_local),
    [&filter_nan, i = 0](int) mutable { return filter_nan[i++]; });

    // ** _norm => _local **  (Nan filtered) //


    // inverse localization starting from the center of the scene
    vector<double> lon = {offset_lon};
    vector<double> lat = {offset_lat};

    vector<double> row_start(1);
    vector<double> col_start(1);
    vector<double> alt_start(1);


    tie(row_start, col_start, alt_start) = inverse_loc(lon, lat, alt_local);//alt_start useless
    ...
    vector<double> delta_col(row_local.size());
    vector<double> delta_row(row_local.size());

    for (int i = 0;i<int(col_local.size());++i){
        delta_col[i] = col_local[i] - col_start[0];
        delta_row[i] = row_local[i] - row_start[0];
    }

    // ground coordinates (latitude and longitude) of each point
    lon.resize(delta_col.size(),offset_lon);
    lat.resize(delta_row.size(),offset_lat);

    vector<double> iter_;
    vector<double> lon_iter;
    vector<double> lat_iter;
    vector<double> alt_iter;

    vector<double> drow_dlat;
    vector<double> drow_dlon;
    vector<double> dcol_dlat;
    vector<double> dcol_dlon;

    vector<double> det;
    vector<double> delta_lon;
    vector<double> delta_lat;

    vector<double> row_estim;
    vector<double> col_estim;
    vector<double> alt_estim;

    // while the required precision is not achieved
    while ((max_abs(delta_col) > eps || max_abs(delta_row) > eps) && iteration < nb_iter_max){

        // list of points that require another iteration
        iter_.clear();//index of element still needing computing
        lon_iter.clear();
        lat_iter.clear();
        alt_iter.clear();

        // for loop on coord
        for (int i = 0; i < int(delta_col.size()); ++i) {
            if (abs(delta_col[i]) > eps || abs(delta_row[i]) > eps) {
                iter_.push_back(i);
                lon_iter.push_back(lon[i]);
                lat_iter.push_back(lat[i]);
                alt_iter.push_back(alt_local[i]);
            }
        }


        size_t len_iter = iter_.size();

        dcol_dlon.resize(len_iter,0.0);
        dcol_dlat.resize(len_iter,0.0);
        drow_dlon.resize(len_iter,0.0);
        drow_dlat.resize(len_iter,0.0);

        // partial derivatives
        tie(dcol_dlon, dcol_dlat, drow_dlon, drow_dlat) = compute_loc_inverse_derivates(
            lon_iter, lat_iter, alt_iter
        );

        
        det.resize(len_iter,0.0);
        delta_lon.resize(len_iter,0.0);
        delta_lat.resize(len_iter,0.0);

        for (int i =0;i<int(len_iter);++i){
            det[i] = dcol_dlon[i] * drow_dlat[i] - drow_dlon[i] * dcol_dlat[i];

            delta_lon[i] = (drow_dlat[i] * delta_col[iter_[i]] - dcol_dlat[i] * delta_row[iter_[i]]) / det[i];
            delta_lat[i] = (-drow_dlon[i] * delta_col[iter_[i]] + dcol_dlon[i] * delta_row[iter_[i]]) / det[i];

            // update ground coordinates 
            lon_iter[i] = lon_iter[i]+delta_lon[i];
            lat_iter[i] = lat_iter[i]+delta_lat[i];
        }


        //reconsrtruction of lon/lat
        for(int i =0;i<int(iter_.size());++i){
            lon[iter_[i]] = lon_iter[i];
            lat[iter_[i]] = lat_iter[i];
        }

        // inverse localization
        row_estim.resize(len_iter,0.0);
        col_estim.resize(len_iter,0.0);
        alt_estim.resize(len_iter,0.0);

        tie(row_estim, col_estim, alt_estim) = inverse_loc(lon_iter, lat_iter, alt_iter);
        ...
        for (int i =0;i<int(len_iter);++i){
            delta_col[iter_[i]] = col_local[iter_[i]] - col_estim[i];
            delta_row[iter_[i]] = row_local[iter_[i]] - row_estim[i];
        }
        ++iteration;
    }

    int i_lon = 0;
    for (int i =0;i<int(long_out.size());++i){
        if(filter_nan[i]){
            long_out[i] = lon[i_lon];//long_out[filter_nan] = lon
            lat_out[i] = lat[i_lon];
            ++i_lon;
        }else{
    ...
    return make_tuple(long_out, lat_out, alt_local);
}
