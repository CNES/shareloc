...
class Grid:
    ...
    def direct_loc_h(self, row, col, alt, fill_nan = False):
        ...
        mats = [self.gld_lon[grid_index_up:grid_index_down+1, :, :],
    ...
    def direct_loc_grid_h(self, row0, col0, steprow, stepcol, nbrow, nbcol, alt):
        ...
        alt_up = self.alts_down[grid_index_up]
        alti_coef = (alt - alt_down) / (alt_up - alt_down)
        mats =  [self.gld_lon[grid_index_up:grid_index_down+1,:,:],self.gld_lat[grid_index_up:grid_index_down+1,:,:]]
        position = np.zeros(3)
        position[2] = alt
    ...
    def inverse_partial_derivative(self, row, col, alt=0):
        ...
        h_x = (alt - self.alts_down[grid_index_down])/(self.alts_down[grid_index_up]-self.alts_down[grid_index_down])
